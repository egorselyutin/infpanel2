...
/* Единое правило для шрифта на кастомной кнопке и внутри вложенных тегов Streamlit */
.portal-btn,
div.stButton > button,
...
/* ИЗМЕНЕНО: hover-цвет заменён с rgb(255,75,75) на rgb(41,128,185) */
...
/* ИЗМЕНЕНО: active-цвет заменён — шрифт синий вместо белого, фон светло-серый вместо красного */
...
/* ЭФФЕКТ НАВЕДЕНИЯ: Кнопка слегка "приподнимается", подчеркивая интерактивность */
.portal-btn:hover, div.stButton > button:hover {{
...
/* ЭФФЕКТ НАЖАТИЯ: Кнопка физически "вдавливается" под пальцем/курсором */
.portal-btn:active, div.stButton > button:active {{
...
/* Выравнивание контейнера Streamlit-кнопки на главной странице */
div.stButton {{
    display: flex;
...
/* ИЗМЕНЕНО: border-left заменён с rgb(255,75,75) на rgb(41,128,185) */
.contacts-info-card {{
...
/* ДОБАВЛЕНО: active-состояние для кнопки «Возврат на главную страницу» */
...
# =============================================================================
# 6. ФУНКЦИИ ПОДГОТОВКИ ДАННЫХ И SVG КАРТЫ
# =============================================================================
@st.cache_data
...
def load_np_data(file_name):
    ...
    for d in search_dirs:
        ...
        if os.path.exists(exact):
            return pd.read_excel(exact)
    for d in search_dirs:
        try:
            for fname in os.listdir(d):
                if fname.lower() == file_name.lower():
                    return pd.read_excel(os.path.join(d, fname))
        except Exception:
            pass
    import glob
    for d in search_dirs:
...
if not display_df.empty:
    if "Численность населения, чел." in display_df.columns:
        display_df["Численность населения, чел."] = display_df["Численность населения, чел."].astype(float).round(0).astype(int)
    if "Действующие ФП" in display_df.columns:
        display_df["Действующие ФП"] = display_df["Действующие ФП"].astype(float).round(1)
        ...
        if not region_row.empty:
            ...
            if df_np_all is None:
                ...
                for d in [cwd, script_dir]:
                    try:
                        found_files += [os.path.join(d, f) for f in os.listdir(d) if f.lower().endswith(".xlsx")]
                    except Exception:
                        pass
                files_str = ", ".join(found_files) if found_files else "(xlsx файлы не найдены)"
                st.error(f"❌ Файл {NP_FILE} не найден. cwd={cwd}. Найденные xlsx: {files_str}")
            else:
                mask = df_np_all["Район"].astype(str).str.strip() == str(region_name).strip()
                df_np_region = df_np_all[mask][NP_COLS].copy()
                df_np_region = df_np_region.sort_values("Населенный пункт").reset_index(drop=True)
                if "Численность населения, чел." in df_np_region.columns:
                    df_np_region["Численность населения, чел."] = df_np_region["Численность населения, чел."].apply(
                        lambda x: int(round(float(x))) if pd.notna(x) else 0)
                if "Действующие ФП" in df_np_region.columns:
                    df_np_region["Действующие ФП"] = df_np_region["Действующие ФП"].apply(
                        lambda x: round(float(x), 1) if pd.notna(x) else 0.0)
                if "КФД, в баллах" in df_np_region.columns:
                    df_np_region["КФД, в баллах"] = df_np_region["КФД, в баллах"].apply(
                        lambda x: round(float(x), 1) if pd.notna(x) else "")

            b64_np_excel = ""
            if not df_np_region.empty:
                buffer_np = io.BytesIO()
                with pd.ExcelWriter(buffer_np, engine='xlsxwriter') as writer:
                    df_np_region.to_excel(writer, index=False, sheet_name='Населенные пункты')
                    worksheet = writer.sheets['Населенные пункты']
                    
                    for idx, col in enumerate(df_np_region.columns):
                        max_len = max(df_np_region[col].astype(str).map(len).max(), len(str(col))) + 2
                        worksheet.set_column(idx, idx, max_len)
                        
                b64_np_excel = base64.b64encode(buffer_np.getvalue()).decode()
...
table thead tr th {{
    ...
    background-color: #f8f9fa; color: #000; text-align: center;
...
function recalcSums() {{
    ...
    if (fpCell)  fpCell.innerText  = sumFp.toFixed(1).replace(".0", "");
